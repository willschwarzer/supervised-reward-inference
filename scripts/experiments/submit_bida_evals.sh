...
for blk in "${BLOCKS[@]}"; do
  case "$blk" in
    ...
    1)
      PPDATA_SCRIPT="scripts/experiments/run_ppdata_eval_bida.sh"
      PPDATA_NUM_JOBS=270
      # sbatch --array=0-$(($PPDATA_NUM_JOBS-1))%30 <<EOF
      sbatch <<EOF
#!/bin/bash
#SBATCH -J ppdata_eval_bida
...
EOF
    ...
    3)
      DATA_SCRIPT="scripts/experiments/run_data_rfpp_eval_bida.sh"
      DATA_NUM_JOBS=270
    #   sbatch --array=0-$(($DATA_NUM_JOBS-1))%30 <<EOF
    sbatch <<EOF
#!/bin/bash
#SBATCH -J data_eval_bida
#SBATCH --dependency=afterok:34203979
...
EOF
    ...
    4)
      N_SCRIPT="scripts/experiments/run_n_rfpp_eval_bida.sh"
      N_NUM_JOBS=90
      # sbatch --array=0-$(($N_NUM_JOBS-1))%30 <<EOF
      sbatch <<EOF
#!/bin/bash
#SBATCH -J n_eval_bida
...
EOF
    ...
    5)
      ADJUST_SCRIPT="scripts/experiments/run_adjust_rfpp_eval_bida.sh"
      ADJUST_NUM_JOBS=180
      sbatch <<EOF
#!/bin/bash
#SBATCH -J adjust_eval_bida
...
EOF
    ...
    6)
      # NOISE_SCRIPT="scripts/experiments/run_noise_rfpp_eval_bida.sh"
      NOISE_SCRIPT="scripts/experiments/run_noise_rfpp_eval_bida_2000.sh"
      NOISE_NUM_JOBS=210
      # sbatch --array=0-$(($NOISE_NUM_JOBS-1))%30 <<EOF
      sbatch <<EOF
#!/bin/bash
#SBATCH -J noise_eval_bida

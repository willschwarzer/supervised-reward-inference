...
GEN_SCRIPT_1="scripts/rl/16cpu_array_generate_reach_circling.sh"
NUM_JOBS_GEN_1=30

gen_job1_id=$(sbatch --array=0-$(($NUM_JOBS_GEN_1-1))%15 $GEN_SCRIPT_1 | awk '{print $4}')

SRI_SCRIPT="scripts/experiments/run_data_rfpp_no_inf.sh"
NUM_JOBS_SRI=270

# Launch the SRI job with dependencies on the gen jobs
# sri_job_id=$(sbatch --array=0-$(($NUM_JOBS_SRI-1))%30 <<EOF
sri_job_id=$(sbatch --array=0-$(($NUM_JOBS_SRI-1))%30 --dependency=afterok:$gen_job1_id:26441558 <<EOF
#!/bin/bash
#SBATCH -n 1
#SBATCH -c 2
#SBATCH --mem=40G
...
RL_SCRIPT="scripts/experiments/run_data_rfpp_rl.sh"

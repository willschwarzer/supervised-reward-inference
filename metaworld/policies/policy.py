...
def move(from_xyz, to_xyz, p):
    ...
    # Check type and use appropriate library for the "any" operation
    # breakpoint()
    if hasattr(response, 'type') and 'torch' in str(type(response)):
        if torch.any(torch.abs(response) > 1.0):
            warnings.warn(

"""A set of reward utilities written by the authors of dm_control. (Enhanced by SRI authors with batching.)"""
...
def _sigmoids_batch(x, value_at_1, sigmoid):
    """Returns 1 when `x` == 0, between 0 and 1 otherwise.
    ...
    if sigmoid == "gaussian":
        ...
        scaled_x = x * scale
        return torch.where(torch.abs(scaled_x) < 1, (1 + torch.cos(torch.pi * scaled_x)) / 2, torch.tensor(0.0))
...
#     # again, margin is a tensor
#     else:
#         d = torch.where(x < lower, lower - x, x - upper) / margin
#         value = torch.where(in_bounds, torch.tensor(1.0), _sigmoids_batch(d, value_at_margin, sigmoid))
...
def tolerance_batch(
    ...
    Args:
        ...
        at one or both ends, or they can be equal to one another if the target
        value is exact.
        margin: A PyTorch tensor with the same shape as `x`. Parameter that controls
        how steeply the output decreases as `x` moves out-of-bounds.
        * If `margin == 0` then the output will be 0 for all values of `x`
            outside of `bounds`.
        * If `margin > 0` then the output will decrease sigmoidally with
            increasing distance from the nearest bound.
    ...
    Raises:
        ValueError: If `bounds[0] > bounds[1]`.
        ValueError: If `margin` contains negative values.
    """
    # lower, upper = bounds
    # if lower > upper:
    #     raise ValueError("Lower bound must be <= upper bound.")
    # if (margin < 0).any():
    #     raise ValueError(f"`margin` must be non-negative. Current values: {margin[margin < 0]}")

    # in_bounds = torch.logical_and(lower <= x, x <= upper)
    # zero_margin = margin == 0
    # value = torch.where(in_bounds, torch.tensor(1.0), torch.tensor(0.0))
    # non_zero_margin = ~zero_margin
    # if non_zero_margin.any():
    #     d = torch.where(x < lower, lower - x, x - upper) / margin[non_zero_margin]
    #     value[non_zero_margin] = _sigmoids(d, value_at_margin, sigmoid)
    ...
    in_bounds = torch.logical_and(lower <= x, x <= upper)
    value = torch.where(in_bounds, torch.tensor(1.0, dtype=x.dtype, device=x.device), torch.tensor(0.0, dtype=x.dtype, device=x.device))

    # Apply sigmoid only where margin is non-zero and x is out-of-bounds
    out_of_bounds = ~in_bounds
    non_zero_margin = margin > 0
    applicable = non_zero_margin & out_of_bounds

    if applicable.any():
        d = torch.zeros_like(x)  # Initialize d with zeros
        # Calculate d only for applicable cases to avoid division by zero
        d[applicable] = torch.where(x[applicable] < lower, lower - x[applicable], x[applicable] - upper) / margin[applicable]
        sigmoid_value = _sigmoids_batch(d, value_at_margin, sigmoid)
        value = torch.where(applicable, sigmoid_value, value)

    return value


def inverse_tolerance_batch(x, bounds=(0.0, 0.0), margin=0.0, sigmoid="reciprocal"):
    """Returns 0 when `x` falls inside the bounds, between 1 and 0 otherwise.
    ...
    Raises:
        ...
        ValueError: If `margin` is negative.
    """
    bound = tolerance_batch(
        x, bounds=bounds, margin=margin, sigmoid=sigmoid, value_at_margin=0
    )
    return 1 - bound


def rect_prism_tolerance_batch(curr, zero, one):
    """Computes a reward if curr is inside a rectangular prism region.
    ...
    y_scale = (curr[:, 1] - zero[1]) / diff[1]
    z_scale = (curr[:, 2] - zero[2]) / diff[2]
    return torch.where(in_prism, x_scale * y_scale * z_scale, torch.tensor(1.0))


def hamacher_product_batch(a, b):
    """The hamacher (t-norm) product of a and b.

    computes (a * b) / ((a + b) - (a * b))

    Args:
        a (torch.Tensor): 1st term of hamacher product.
        b (torch.Tensor): 2nd term of hamacher product.

    Raises:
        ValueError: a and b must range between 0 and 1

    Returns:
        torch.Tensor: The hammacher product of a and b
    """
    if not ((0.0 <= a).all() and (a <= 1.0).all() and (0.0 <= b).all() and (b <= 1.0).all()):
        raise ValueError("a and b must range between 0 and 1")

    denominator = a + b - (a * b)
    h_prod = torch.where(denominator > 0, (a * b) / denominator, torch.tensor(0.0))

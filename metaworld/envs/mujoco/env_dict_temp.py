...
from metaworld.envs.mujoco.sawyer_xyz.v2.sawyer_pick_place_v2_temp import SawyerPickPlaceEnvV2
from metaworld.envs.mujoco.sawyer_xyz.v2.sawyer_reach_v2_temp import SawyerReachEnvV2
    ...
    for env_name, env_cls in ALL_V2_ENVIRONMENTS.items():
        ...
        def initialize(
            ...
            super(type(env), env).__init__(
                ...
                # no_init_success=no_init_success,
            ...
            env.gripped_start = gripped_start
            env.record_grips = record_grips
            env.max_path_length = horizon
            env.reset()
            # use of reinit_goals is confusing here, but it's only used with legacy-style RL, so usually is not relevant
            env._freeze_rand_vec = not reinit_goals and not reinit_obj_pos
            if not env._freeze_rand_vec:
                print("Object will vary positions")
                if goals is not None:
                    print("However, goal will be fixed as", goals)
                else:
                    print("Goal will also vary positions")
            env.reinit_goals = reinit_goals
            if not reinit_goals and not reinit_obj_pos and not multi_obj_pos:
                env._last_rand_vec[:3] = np.array([0.0, 0.6, 0.02])
            elif multi_obj_pos:
                print(f"Obj pos for this env is {env._last_rand_vec[:3]}")
            if seed is not None:
                env.seed(seed)
                np.random.set_state(st0)

        d["__init__"] = initialize
        og_env_name = re.sub(r"(^|[-])\s*([a-zA-Z])",

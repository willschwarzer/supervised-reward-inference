...
def get_algorithm_name(alg):
    if alg == 'given_rewards':
        # return 'Algorithm 1'
        # return 'Inverse planning (known rewards)'
        return 'IP (known rewards)'
    elif alg == 'sri_policy':
        return 'SRI (policy input)'
    elif alg == 'boltzmann_planner':
        # return 'Boltzmann'
        # return 'Inverse planning (Boltzmann)'
        return 'IP (Boltzmann)'
    elif alg == 'optimal_planner':
        # return 'Optimal'
        # return 'Inverse planning ("optimal")'
        # return 'IP (optimal)'
        # return 'IP (assumes optimality)'
        return 'IP (assumed optimality)'
        ...
        # return 'Inverse planning (estimated bias)'
        return 'IP (estimated bias)'
...
def get_algorithm_hatch(alg):
    if alg in ['em_without_init', 'joint_without_init', 'joint_no_rewards']:
        return '/'
    elif alg in ['given_rewards', 'sri_policy', 'boltzmann_planner', 'optimal_planner', 'em_with_init', 'no_rewards', 'joint_with_init', 'vi_inference']:
        return None
    else:
        raise ValueError('Unknown algorithm ' + alg)
...
def drop_irrelevant_data(data, args):
    ...
    data.drop(columns=cols_to_skip, inplace=True, errors='ignore')
...
def graph(args):
    ...
    if num_cols == 1:
        ...
        # Keep legend outside the plotting area for multi-panel layouts.
        fig.legend(map(alg_to_bar.get, args.algorithm),
                   map(get_algorithm_name, args.algorithm),
                   loc='lower center',
                   bbox_to_anchor=(0.5, -0.02),
                   ncol=min(len(args.algorithm), 5),
                   fontsize=12,
        )

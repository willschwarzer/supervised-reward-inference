...
from sri.utils import convert_chai_rollouts, generate_gadget_demonstration
...
from sri.reward_inference.object_env import ObjectEnv
import ray
import wandb
...
def parse_args():
    ...
    parser.add_argument(
        "--data",
        type=str,
        default="grid_regression_with_weights_chai_100000",
    ...
    parser.add_argument(
        ...
        "-sm",
        type=str,
        default="ground_truth_phi_test.parameters",
        help="location of saved model to load",
    )
    ...
    parser.add_argument(
        "--single-move", action=argparse.BooleanOptionalAction, help="Only for ring env"
    )
    parser.add_argument(
        "--num-rollouts-per-agent",
        default=10000,
        type=int,
    ...
    parser.add_argument(
        ...
        default=400000,
        type=int,
        help="Number of iterations to run reinforcement learning",
    )
    parser.add_argument(
        "--adv-its",
        default=1000000,
        type=int,
        help="Number of iterations to run adversarial algs",
    )
    ...
    parser.add_argument(
        "--percentile-evaluation", default=True, action=argparse.BooleanOptionalAction
    )
    parser.add_argument("--agent-save-dir", default="imitation_agents", type=str)
    parser.add_argument(
        "--save-agents", default=True, action=argparse.BooleanOptionalAction
    )
    parser.add_argument(
        "--overwrite-agents", default=False, action=argparse.BooleanOptionalAction
    )
    parser.add_argument(
        "--methods",
        type=str,
    ...
    parser.add_argument(
        ...
        action=argparse.BooleanOptionalAction,
        default=False,
        help="Whether GAIL and AIRL only get observations/states",
    )
...
def evaluate_ground_truth(env, rl_its, num_eval_episodes, save_dest):
    ...
    mean_ground_truth_ret, _ = evaluate_policy(
        agent, env, n_eval_episodes=num_eval_episodes
    )
...
def evaluate_sirl(pred_env, ground_truth_env, rl_its, num_eval_episodes, save_dest):
    ...
    mean_ground_truth_pred_agent_ret, _ = evaluate_policy(
        pred_agent, ground_truth_env, n_eval_episodes=num_eval_episodes
    )
    # Save agent
...
def evaluate_adversarial(
    rollouts, env, alg, adv_its, num_eval_episodes, save_dest, ifo=False
):
    ...
    learner = train_adversarial(rollouts, env, alg, adv_its, ifo)
...
def train_adversarial(rollouts, venv, alg, adv_its, ifo, rl_config):
    trainer = GAIL if alg == "gail" else AIRL
...
def evaluate_bc(rollouts, env, bc_epochs, num_eval_episodes, save_dest):
    ...
    policy = train_bc(rollouts, env, bc_epochs)
    mean_ret, _ = evaluate_policy(
        bc_trainer.policy, env, n_eval_episodes=num_eval_episodes
    )
    bc_trainer.policy.save(os.path.join(save_dest, "bc", str(mean_ret)))
    return mean_ret


def train_bc(rollouts, venv, bc_epochs, rl_config):
    policy_kwargs = dict(
        net_arch=dict(
            pi=[rl_config.policy_latent_dim] * rl_config.policy_num_layers,
            vf=[rl_config.policy_latent_dim] * rl_config.policy_num_layers,
...
def evaluate_sqil(rollouts, env, sqil_its, num_eval_episodes, save_dest):
    ...
    mean_ret, _ = evaluate_policy(
        sqil_trainer.policy, vec_env, n_eval_episodes=num_eval_episodes
    )
    return mean_ret
...
def main(args):
    ...
    if "grid" in args.env:
        raise NotImplementedError(
            "You need to deal with obs_size in convert_chai_rollouts: it needs to be obs_size//num_classes)"
        )
        obs_size = 4 * 25
        horizon = 150
        dtype = int
    ...
    if "grid" in args.env or "space" in args.env:
        ...
        state_batches = (
            F.one_hot(state_batches_int, num_classes=4)
            .view(-1, args.num_rollouts_per_agent, 150, 100)
            .to(torch.float)
        )
    else:
        state_batches = torch.Tensor(
            states.reshape(-1, args.num_rollouts_per_agent, horizon, obs_size)
        )
    ...
    for idx in range(len(state_batches) - 1, -1, -1):
        ...
        if "gadget" in demonstration_types:
            gadget_demo = generate_gadget_demonstration(weights).unsqueeze(
                0
            )  # Generate gadget demo
            demos.append(gadget_demo)
            if "gail" in methods or "airl" in methods or "bc" in methods:
                raise NotImplementedError(
                    "Can't use GAIL, AIRL or BC with gadget demos yet, sorry mate :("
                )
        ...
        # rollout_batch = rollouts[idx*args.num_rollouts_per_agent:idx*args.num_rollouts_per_agent+args.num_rollouts_to_use]
            ...
            idx * args.num_rollouts_per_agent + args.num_rollouts_to_use, len(rollouts)
        )
        if end == len(rollouts):
            print(
                "Notice: not enough rollouts to fill the batch for adversarial methods"
            )
        ...
        if "random_rew" in methods:
            random_rew_rets = ray.get(
                [
                    evaluate_random_rew.remote(
                        ground_truth_env,
        ...
        if "ground_truth" in methods:
            ground_truth_rets = ray.get(
                [
                    evaluate_ground_truth.remote(
                        ground_truth_env, args.rl_its, args.num_eval_episodes, save_dest
                    )
        ...
        if args.percentile_evaluation:
            ...
            if "ground_truth" in methods:
                ground_truth_percentiles = np.digitize(
                    ground_truth_rets, random_rets_sorted
                )
